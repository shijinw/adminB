  ...
  for (let i = 0; i < menuList.length; i++) {
    ...
    if (item['parent_id'] === parentId) {
      if (item['menu_type'] === 1) {
        ...
        menuInfo['name'] = item['menu_name']
        menuInfo['meta'] = {'title': item['menu_title'], 'icon': item['menu_icon'], 'hideInMenu': (parseInt(item['menu_hide']) !== 0)}
        ...
        menuInfo['meta'] = {'title': item['menu_title'], 'icon': item['menu_icon'], 'notCache': (parseInt(item['menu_cache']) !== 0),'hideInMenu': (parseInt(item['menu_hide']) !== 0)}

        let children = menuRefactoring(menuList, item['id'].toString())
