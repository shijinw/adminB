         ...
         <div v-if="vasable">
           <Row>
            ...
            <FormItem label="是否上架" prop="status">
              ...
              <Row>
              <Images ref="imgs" :text='formValidate.image' />
              </Row>
              <Row>
                 <Col>
...
<script>
import { putGoods, goodsDetails } from '@/api/info'
...
export default {
  ...
  methods: {
    handleSubmit (name) {
      this.formValidate.image = this.$refs.imgs.uploadList3[0].url
      this.$refs[name].validate((valid) => {
        if (valid) {
          putGoods(this.formValidate).then(res => {
    ...
    getPutData () {
      goodsDetails({ id: this.$route.query.id }).then(res => {
        let rep = res.data
        if (rep.code === 200) {
          this.formValidate.name = rep.data.name
          this.formValidate.type = rep.data.type
          this.formValidate.status = rep.data.status
          this.formValidate.introduce = rep.data.introduce
          this.formValidate.integral_price = rep.data.integral_price
          this.formValidate.number = rep.data.integral_price
          this.formValidate.type_id = rep.data.type_id
          this.formValidate.end_time = rep.data.end_time
          this.formValidate.range = rep.data.range
          this.formValidate.image = rep.data.image
          this.$refs.imgs.uploadList3[0].url = this.formValidate.image
