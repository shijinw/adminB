    ...
    <Card>
      ...
      <div>
        ...
        <Button v-if="Auth.top[0]"
                ...
                @click="addTag">{{Auth.top[0].menu_title}}</Button>
      </div>
      <Divider />
      <Table size="small"
             :columns="columns"
      ...
      <Modal v-model="modal"
        ...
        <div>
          <Form ref="formValidate"
            ...
            <FormItem label="分类名称"
                      prop="type_name">
              <Input v-model="formValidate.type_name"
                     placeholder="标签名称"></Input>
              <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：请输入分类名称</p>
            </FormItem>
            <FormItem label="是否展示"
                      prop="status">
...
import { eduTypeList, addEduType, putEduType, delEduType } from '@/api/Educode';
export default {
  data() {
    return {
      data: [],
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.type == 1) {
            addEduType(this.formValidate).then(res => {
            ...
            putEduType(this.formValidate).then(res => {
              if (res.data.code === 200) {
    ...
    publicFun(key, row) {
      if (key === 'B') {
        ...
        this.formValidate.status =
          typeof row.status === 'number' ? row.status.toString() : row.status
        this.modal = true
      } else if (key === 'C') {
        this.$Modal.confirm({
          title: '温馨提示',
