      ...
      >
        ...
        <Row>
          <FormItem label="资讯内容" prop="content">
            <editor ref="editor" :retitle="'Zeditop'"  :value.sync="formValidate.content"></editor>
            <p
              style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;"
            >
              提示：输入详细的资讯内容
        ...
        <Row>
          <Col :span="12">
            <FormItem label="作者" prop="author" style="margin-left:10px">
              <Input
          ...
          <Col :span="12">
            <FormItem label="是否热点" prop="is_hot">
              <RadioGroup
        ...
        <Row>
          <Col :span="12">
            <FormItem label="是否头条" style="margin-left:20px" prop="is_head">
              <RadioGroup
                v-model="formValidate.is_head"
                style="margin-left:30px"
              >
        ...
        <FormItem>
          <div style="text-align: center;">
            <Button
              @click="resets('formValidate')"
              style="margin-left: 8px;width:110px;height:40px"
              >重置</Button
            >
            <Button
              type="primary"
...
<script>
import { NewsTypeLevel, getNews, putNews } from "@/api/Consult";
...
export default {
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.formValidate.news_id = this.$route.query.id;
      this.formValidate.image = this.$refs.images.uploadList3[0].url;
     
      this.$refs[name].validate(valid => {
        if (valid) {
          putNews(this.formValidate).then(res => {
            let rep = res.data;
            if (rep.code === 200) {
    ...
    getData() {
      getNews({ news_id: this.$route.query.id }).then(res => {
        let rep = res.data;
        if (rep.code === 200) {
        
          this.$refs.images.uploadList3[0].url = rep.data.image;
          ...
          this.formValidate.jumphref = rep.data.jumphref;
          this.formValidate.author = rep.data.author;
          this.formValidate.is_hot = rep.data.is_hot;
          this.formValidate.parent_id = rep.data.parent_id;
          this.formValidate.is_head = rep.data.is_head;
          this.formValidate.type_id = rep.data.type_id;
          this.valueList = [rep.data.parent_id, rep.data.type_id];
        }
      });
    }
  },
