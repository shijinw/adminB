      ...
      <div style="margin-bottom:20px">
       <Button type="default" shape="circle" icon="md-sync" @click="seachQ"></Button>
        ...
        <Divider type="vertical" />
          ...
          <Button v-if="Auth.top[0]" :type='Auth.top[0].menu_type' style="margin-left: 10px;" @click="addTags">{{Auth.top[0].menu_title}}</Button>
      </div>
...
<script>
import { addPlan, planList, putPlan, delPlan, statusPlan, delVideo, planCouList, getVideoList, addPlanDetail } from '@/api/studyplan'
...
export default {
  ...
  methods: {
    ...
    removeVideo (row) {
      delVideo({ plan_id: this.formDalidate.plan_id, days: this.formDalidate.days, video_id: row.id }).then(res => {
        let rep = res.data
        if (rep.code === 200) {
    ...
    handleSubmit () {
      this.$refs.formValidate.validate((valid) => {
        addPlan(this.formValidate).then(res => {
          let rep = res.data
          if (rep.code === 200) {
            this.modal2 = false
    ...
    handleSubmit2 () {
      putPlan(this.formCalidate).then(res => {
        let rep = res.data
        if (rep.code === 200) {
          this.modal3 = false
    ...
    getvideo (id) {
      planCouList({ plan_id: id, days: this.formDalidate.days }).then(res => {
        let rep = res.data
        if (rep.code === 200) {
    ...
    publicFun (key, row) {
      switch (key) {
        case 'b':
          statusPlan({ id: row.id, status: row.status === 1 ? '2' : '1' }).then(res => {
            let rep = res.data
            if (rep.code === 200) {
              this.getData(this.formlate)
            }
