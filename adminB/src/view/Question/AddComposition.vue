      ...
      <Form ref="formValidate"
        ...
        <Row>
          ...
          <FormItem label="答题时间"
            ...
            <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：题限制答题时间1-240</p>
          </FormItem>
          </Col>
          <Col :span="12">
          <FormItem label="试卷总分"
                    prop="mock_score">
            <Input type="number"
                   v-model="formValidate.mock_score"
                   placeholder="输入试卷总分"
                   style="width: 100%" />
        ...
        <Row>
          ...
          <FormItem label="每题分值"
            ...
            <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：输入每题分值1-100</p>
          </FormItem>
...
import { getPlate, addPaperMock } from '@/api/question';
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs.formValidate.validate(valid => {
        if (valid) {
          addPaperMock(this.formValidate).then(res => {
            let rep = res.data
            if (rep.code === 200) {
