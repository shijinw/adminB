  ...
  <div>
    <Card>
      ...
      <div style="display: flex;flex-wrap: wrap;width: 100%; align-items: center;">
        ...
        <Select v-model="formValidate.plate_id"
                clearable
                placeholder="版块名称"
                style="width:100px">
          <Option v-for="item in formatlate"
                  :value="item.id"
                  :key="item.id">{{
            item.plate_name
          }}</Option>
        ...
        <div style="float:right">
          <Button type="primary"
                  @click="seachKnow"
                  style="margin-left: 10px;">筛选</Button>
...
export default {
  ...
  methods: {
    ...
    ADDsles() {
      ...
      addPaperQuestion({ question_id: arr, paper_id: this.rows.id }).then(
        res => {
          let rep = res.data;
          if (rep.code === 200) {
            this.getData1(this.rows);
    ...
    publicFun(key, row) {
      ...
      if (key === "e") {
        statusPaper({ id: row.id, status: row.status === 1 ? "2" : "1" }).then(
          res => {
            let rep = res.data;
            if (rep.code === 200) {
              this.$Message.success(res.data.msg);
      ...
      if (key === "N") {
        paperContentExport({ paper_id: row.id }).then(res => {
    ...
    changevalue() {
      testPaperList(this.Manage).then(res => {
        let rep = res.data;
        if (rep.code === 200) {
          this.paperNum.selesall = rep.data.question_num;
          this.paperNum.seles = rep.data.at_question_num;
    ...
    selectionCancel(arr, row) {
      this.Sele_id.splice(
        this.Sele_id.findIndex(v => v === row.id),
        1
      );
      if (this.paperNum.seles) {
        this.chnageChaked = this.Sele_id.length + this.paperNum.seles;
