<template>
  <div>
   <Card>
      ...
      <Row>
          <Col :span="12" >
            <FormItem label="图书名称" prop="name" >
               <Input  v-model="formValidate.name" placeholder="输入图书名称" style="width: 100%" />
               <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：输入图书名称</p>
            </FormItem>
          </Col>
           <Col :span="12">
            <FormItem label="出版社" prop="pub_company" >
               <Input v-model="formValidate.pub_company" placeholder="输入出版社" style="width: 100%" />
          ...
          <Row>
            ...
            <FormItem label="价格" prop="price">
             <Input type="number"  v-model="formValidate.price" placeholder="输入图书价格" style="width: 100%" />
              <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：输入图书价格</p>
            </FormItem>
          </Col>
          <Col :span="12">
            <FormItem label="支付价格" prop="pay_price" >
            <Input type="number"  v-model="formValidate.pay_price" placeholder="输入支付价格" style="width: 100%" />
          ...
          <Row>
            ...
            <FormItem label="是否上架" prop="status" >
               <Select v-model="formValidate.status" clearable placeholder="是否上架">
                <Option v-for="item in Tagslists" :value="item.id" :key="item.id">{{ item.name }}</Option>
              </Select>
              <p style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;">提示：选择是否上架</p>
            </FormItem>
          </Col>
           <Col :span="12">
           <FormItem label="是否预售" prop="is_presell" >
             <Select v-model="formValidate.is_presell" clearable placeholder="是否预售">
          ...
          <Row>
          <Col :span="12" >
            <FormItem label="图书作者" prop="author" >
               <Input  v-model="formValidate.author" placeholder="输入图书作者" style="width: 100%" />
    ...
    <Row style="margin-left:10px">
          <Col :span="12">
            <FormItem label="PC封面图" prop="pc_img">
              <div class="course_img">
                <div class="demo-upload-list" >
                  <template>
                    <img :src="formValidate.pc_img">
                    <div class="demo-upload-list-cover">
                      <Icon type="ios-eye-outline" @click.native="handleView(formValidate.pc_img)"></Icon>
                      <Icon type="ios-trash-outline" @click.native="handleRemove3(formValidate.pc_img)"></Icon>
                    </div>
                  </template>
                </div>
                <Upload
 ...
 <Col :span="12">
            <FormItem label="H5封面图" prop="h5_img">
              <div class="course_img">
                <div class="demo-upload-list" >
                  <template>
                    <img :src="formValidate.h5_img">
                    <div class="demo-upload-list-cover">
                      <Icon type="ios-eye-outline" @click.native="handleView4(formValidate.h5_img)"></Icon>
                      <Icon type="ios-trash-outline" @click.native="handleRemove4(formValidate.h5_img)"></Icon>
                    </div>
                  </template>
                </div>
                <Upload
              ...
              <div class="course_img">
                <div class="demo-upload-list" >
                  <template>
                    ...
                    <div class="demo-upload-list-cover">
                      <Icon type="ios-eye-outline" @click.native="handleView5(formValidate.introduce)"></Icon>
                      <Icon type="ios-trash-outline" @click.native="handleRemove5(formValidate.introduce)"></Icon>
                    </div>
                  </template>
                </div>
                <Upload
 ...
 <Col :span="12">
            <FormItem label="图书目录" prop="catalogue">
              <div class="course_img">
                <div class="demo-upload-list" >
                  <template>
                    <img :src="formValidate.catalogue">
                    <div class="demo-upload-list-cover">
                      <Icon type="ios-eye-outline" @click.native="handleView6(formValidate.catalogue)"></Icon>
                      <Icon type="ios-trash-outline" @click.native="handleRemove6(formValidate.catalogue)"></Icon>
                    </div>
                  </template>
                </div>
                <Upload
        ...
        <FormItem>
          <div style="text-align: center;">
            <Button @click="resets('formValidate')" style="width:110px;height:40px">重置</Button>
...
import { putBooks, BooksDetalis } from '@/api/Books'
  ...
  data () {
    return {
      ...
      ruleValidate: {
        ...
        pub_company: [{ required: true, validator: Fomat.Rules.Verifiy().Required }]
      }
    }
  },
  methods: {
    ...
    handleSubmit () {
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          putBooks(this.formValidate).then(res => {
            let rep = res.data
            if (rep.code === 200) {
    ...
    getBook (id) {
      BooksDetalis({ id: id }).then(res => {
        let rep = res.data
        if (rep.code === 200) {
          this.formValidate.name = rep.data.name
          this.formValidate.pc_img = rep.data.pc_img
          this.formValidate.h5_img = rep.data.h5_img
          this.formValidate.price = Number(rep.data.price)
          this.formValidate.pay_price = Number(rep.data.pay_price)
          this.formValidate.introduce = rep.data.introduce
          this.formValidate.catalogue = rep.data.catalogue
          this.formValidate.status = rep.data.status
          this.formValidate.is_presell = rep.data.is_presell
          this.formValidate.pub_company = rep.data.pub_company
