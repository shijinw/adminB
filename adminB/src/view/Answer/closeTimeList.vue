<template>
  <div>
     <Card>
       <p slot="title">答疑追问按钮时间列表</p>
        <Button type='primary' v-if="Auth.top" @click="ADDlist" style="margin: 10px 0;">{{Auth.top[0].menu_title}}</Button>
    ...
    <Modal v-model="modal0"  mask :title="title" footer-hide>
      <div>
        <Form ref="AddFrom" :model="AddFrom" :rules="AddFromvalue" :label-width="80">
     <FormItem label="追问时间"  prop="answer_time" style="margin-top:20px">
        <Input type="number" v-model="AddFrom.answer_time"  placeholder="输入追问时间 /天"  style="width: 100%"/>
     </FormItem>
       </Form>
       <div style="text-align: center;">
            <Button type="primary" @click="sumbit" style="width:110px;height:40px">提 交</Button>
       </div>
      </div>
    </Modal>
  </div>
...
<script>
import { closeTimeList, userPageRuleList, addCloseTime, putCloseTime, statusCloseTime } from '@/api/Answer'
...
export default {
  ...
  methods: {
    ...
    getData () {
      closeTimeList(this.cursefrom).then(res => {
        let rep = res.data
        if (rep.code === 200) {
    ...
    sumbit () {
      if (this.title === '添加追问时间') {
        this.$refs.AddFrom.validate(valid => {
          if (valid) {
            addCloseTime(this.AddFrom).then(res => {
              let rep = res.data
              if (rep.code === 200) {
        ...
        this.$refs.AddFrom.validate(valid => {
          if (valid) {
            putCloseTime(this.AddFrom).then(res => {
              let rep = res.data
              if (rep.code === 200) {
    ...
    publicFun (key, row) {
      switch (key) {
        case 'B':
          ...
          break
        case 'C':
          statusCloseTime({ id: row.id, status: row.status === 1 ? 2 : 1 }).then(res => {
            let rep = res.data
            if (rep.code === 200) {
              this.getData()
            }
