...
<Divider type="vertical" />
     <Input v-model="cursefrom.mobile" placeholder="手机号" style="width: 80px" />
        <Divider type="vertical" />
        <Input v-model="cursefrom.answer_id" placeholder="ID" style="width: 50px" />
        <Divider type="vertical" />
          <Select v-model="cursefrom.course_id" clearable style="width:80px" placeholder="正课类型">
          <Option v-for="item in formatFile" :value="item.menu_title" :key="item.id">{{ item.menu_title }}</Option>
        </Select>
        <Divider type="vertical" />

             <Select v-model="cursefrom.reply_status" clearable placeholder="回复状态" style="width:100px">
  ...
  <Divider type="vertical" />
            ...
            <Divider type="vertical" />
              <Input v-model="cursefrom.reply_quiz" placeholder="回复内容" style="width: 220px" />
        <Divider type="vertical" />
  <Select v-model="cursefrom.reply_user_id"
                    clearable
                    placeholder="回复老师"
                   style="width:110px;margin-top:15px;margin-left:50px">
              <Option v-for="item in teacherList"
                      :value="item.id"
                      :key="item.id">{{ item.truename }}</Option>
            </Select>
...
<!-- 导出弹层 -->
    ...
    <!-- 导出弹层 -->
    <Modal  v-model="modal1" mask title="查看答疑内容" :width="1100"  >
      <div class="tuer">
      ...
      <div class="content"><strong class="example">章节名称 :</strong>  <span style="color:cornflowerblue;font-size:15px" v-text="fromData.section_name"></span></div>
     ...
     <div class="content"><strong class="example">提问端 :</strong>  <span style="color:cornflowerblue;font-size:15px">{{fromData.quiz_source===1?"手机":"pc端"}}</span></div>
        <div class="content"><strong class="example">回复人姓名 :</strong>  <span style="color:cornflowerblue;font-size:15px" v-text="fromData.reply_user_name"></span></div>
        ...
        <div style="border:1px solid #8888;margin:10px auto;width:96%;border-radius: 15px;" v-if="fromData.quiz">
          <div style="margin:10px 20px"><strong class="example">提问内容 :</strong>  <span style="color:cornflowerblue;font-size:15px;word-wrap: break-word;" v-text="fromData.quiz"></span></div>
        <div class="Icontent" style="width:100%"  v-if="fromData.quiz_image">图片扩展 ：
        ...
        <div style="border:1px solid #8888;margin:10px auto;width:96%;border-radius: 15px;" v-if="fromData.reply_quiz">
        <div style="margin:10px 20px" v-if="fromData.reply_quiz"><strong class="example">回复内容 :</strong>  <span style="color:cornflowerblue;font-size:15px;word-wrap: break-word;" v-html="fromData.reply_quiz"></span>
         <div class="Icontent" style="width:100%"  v-if="fromData.reply_image">图片扩展 ：
         ...
         <div style="border:1px solid #8888;margin:10px auto;width:96%;border-radius: 15px;" v-if="fromData.z_quiz">
       ...
       <div style="border:1px solid #8888;margin:10px auto;width:96%;border-radius: 15px;" v-if="fromData.z_reply_quiz">
...
<script>
import { couAnswerList, userPageRuleList, couAnswerExport, lookCouAnswer, delCouAnswer, allAnswerPond, answerPond,getAnswerUser } from '@/api/Answer'
  ...
  methods: {
      ...
      couAnswerExport(this.Comfrom).then(res => {
        if (res.data.code === 200) {
          if (res.data.data instanceof Array) {
            const params = {
              title: ['ID', '手机号码', '课程名称', '章节名称', '创建时间', '回复人姓名', '回复时间', '提问内容', '回复内容', '追问提问', '追问回复'],
              key: ['id', 'mobile', 'course_name', 'section_name', 'create_time', 'reply_user_name', 'reply_time', 'quiz', 'reply_quiz', 'z_quiz', 'z_reply_quiz'],
              data: res.data.data,
              autoWidth: true,
              filename: '课程答疑EXCEl表'
            }
    ...
    getdata () {
      userPageRuleList({ menu_name: this.$route.name }).then(res => {
        let rep = res.data
        if (rep.code === 200) {
          this.formatFile = rep.data.right.filter(item => item.menu_title !== '导出' && item.menu_title !== '一键转移')
