  ...
  <div>
    <Card>
      ...
      <div>
        ...
        />
        <Button type="primary" @click="seach" style="margin-right: 10px;"
          >筛选</Button
        >
        <Button
          v-if="Auth.top[0]"
        ...
        <div>
          ...
          >
            ...
            <FormItem label="视频名称" prop="video_name">
              <Input
                v-model="formValidate.video_name"
                placeholder="视频名称"
              ></Input>
              <p
                style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;"
            ...
            <FormItem label="视频 vid" prop="VideoId">
              <Input
                v-model="formValidate.VideoId"
                placeholder="视频vid"
              ></Input>
              <p
                style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;"
            ...
            <FormItem label="课程时长" prop="video_time">
              <Input
                v-model="formValidate.video_time"
                readonly
                placeholder="课程时长"
                style="width: 200px;"
              ></Input>
              <Button type="primary" @click="getTime" style="margin-left: 10px"
                >获取时长</Button
              >
              <p
                style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;"
            ...
            <FormItem label="视频别名" prop="video_alias">
              <Input
                v-model="formValidate.video_alias"
                placeholder="视频别名"
              ></Input>
              <p
                style="padding-top: 8px; font-size: 12px; color: #999; line-height: normal;"
      ...
      >
        <div>
          <Form ref="pdfForm" :label-width="80">
...
import { proMjor2tree, proMjorCou3tree, knob2Tree } from "@/api/Linkmenu";
...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file) {
      if (res.code === 200) {
        addHandouts({ id: this.row.id, handouts: res.data.file_url }).then(
          res => {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.type == 1) {
            ...
            putVideo(this.formValidate).then(res => {
              if (res.data.code === 200) {
    ...
    publicFun(key, row) {
      switch (key) {
        ...
        case "f":
          ...
          this.formValidate.class_id = row.class_id;
          this.valueList2 = [row.project_id, row.major_id];
