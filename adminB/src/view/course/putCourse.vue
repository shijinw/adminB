          ...
          <RadioGroup :span="12">
            <FormItem label="在线答疑"
              ...
              <p style="padding-top: 8px; font-size:
               12px; color: #999; line-height: nvbormal;">提示：选择该课程是否有在线答疑</p>
            </FormItem>
          </RadioGroup>
          <Col :span="12">
          <FormItem label="翻转课堂"
...
<script>
import { orgList, getQueList, putCourse, getCourse } from '@/api/course';
  ...
  methods: {
    ...
    handleSubmit(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          putCourse(this.formValidate).then(res => {
      ...
      getCourse({ id: id }).then(res => {
        ...
        if (rep.code === 200) {
          ...
          if (rep.code === 200) {
            ...
            this.formValidate.is_zhengke = rep.data.is_zhengke
            ...
            this.formValidate.course_type = rep.data.course_type.toString()
            this.formValidate.bind_question = rep.data.bind_question
            this.formValidate.is_sprint = rep.data.is_sprint.toString()
