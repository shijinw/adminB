    ...
    <Card style="width: 1000px;">
      ...
      <Form ref="formValidate"
        ...
        <Row>
          ...
          <FormItem label="选择分类"
                    prop="title">
             <Select v-model="formValidate.type_id" clearable placeholder="选择分类" style="width:250px">
...
<script>
import { stuCenTypeList, addStuCen } from "@/api/Consult";
  ...
  methods: {
    ...
    handleSubmit(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          addStuCen(this.formValidate).then(res => {
            let rep = res.data;
            if (rep.code === 200) {
